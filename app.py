...
if os.path.exists(key_path):
    try:
        ...
        # Try to use GPT-4.0-mini
        available_models = list(genai.list_models())
        for m in available_models:
            if m.name == "models/gpt-4.0-mini" and hasattr(m, "supported_methods") and "generateContent" in m.supported_methods:
                model = genai.GenerativeModel(m.name)
                use_dummy_generator = False
                break
        if use_dummy_generator:
            st.warning("⚠️ GPT-4.0-mini not found or not supported. Using dummy generator instead.")
    except Exception as e:
